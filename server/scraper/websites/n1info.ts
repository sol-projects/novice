...
async function n1infoSlovenija(n: number = 5) {
   ...
   for (let i = 0; i < links.length && news.length < n; i++) {
     ...
     const titleText = await link.evaluate((e) => (e as HTMLElement).innerText.trim());
     ...
     if (url) {
       ...
       const authorElement = await articlePage.$('a[rel="author"].fn[target="_blank"]');
       ...
       const content = await Promise.all(
         contentElements.map((e) => e.evaluate((el) => (el as HTMLElement).innerText.trim()))
       );
       const joinedContent = content.join(' ');
       console.log('Content:', joinedContent);
 
       const categoryLinks = await articlePage.$$('a[href^="https://n1info.si/tag"]');
