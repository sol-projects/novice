...
sealed class ParserError{
    ...
    data class ExpectedName(val tokenInfo: TokenInfo) : ParserError()
...
fun parserPrintError(parserError: ParserError) {
    when (parserError) {
        ...
        is ParserError.ExpectedName -> println("Syntax error: expected name of type string, found '${parserError.tokenInfo.lexeme}' on line ${parserError.tokenInfo.startPosition.row}")
