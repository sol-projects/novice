...
public class MainGameScreen implements Screen {
    ...
    private Car car;
    private Car2 car2;
    private boolean isCar2Active = false;
    ...
    public MainGameScreen(SpriteBatch batch, OrthographicCamera camera, Viewport viewport) {
        this.batch = batch;
        this.camera = camera;
        ...
        //tire = GameAssetManager.getInstance().getAssetManager().get(AssetDescriptors.PARTICLE_TIRE);
        car = new Car(atlas.findRegion("Player_red_single"));
    ...
    public void render(float delta) {
        ...
        if (currentState == GameState.PLAYING) {
            ...
            if (isCar2Active) {
                ...
                batch.draw(car2.getTexture(), car2.getBounds().x, car2.getBounds().y, car2.getBounds().width, car2.getBounds().height);
            }
    ...
    private void handleInput() {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.B)) {
            if (!isCar2Active) {
                car2 = new Car2(atlas.findRegion("Player_blue_single"));
                isCar2Active = true;
            }
        }
    ...
    private void handleCollisions() {
        handleRoadSignCollisions(car);
        handleSpeedBoostCollisions(car);
        handleGasCollisions(car);

        if (isCar2Active) {
            handleRoadSignCollisions(car2);
            handleSpeedBoostCollisions(car2);
            handleGasCollisions(car2);
        }

    }


    private void handleRoadSignCollisions(Car car) {
    ...
    private void handleSpeedBoostCollisions(Car car) {
    ...
    private void handleGasCollisions(Car car) {
    ...
    private void drawStatus() {
        ...
        if (isCar2Active) {
            font.draw(batch, "PLAYER 2 HEALTH: " + car2.health, 20, Gdx.graphics.getHeight() - 140);
            font.draw(batch, "PLAYER 2 GAS: " + (int) car.gasRemaining, 20, Gdx.graphics.getHeight() - 180);
